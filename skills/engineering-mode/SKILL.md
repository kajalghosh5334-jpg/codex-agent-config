...
### Reasoning Budget（工程场景）

**目的：防止分析漂移、防止跳过分析直接冲。**

```
Task Understanding 阶段：
  分析 ≤ 30%（Task Understanding 本身），执行 = 0%
  Task Understanding 未输出 → 禁止进入任何执行步骤

执行阶段（用户确认 Task Understanding 后）：
  分析 ≤ 20%（Impact Analysis），执行 ≥ 80%
  Impact Analysis 超过 3 个要点 → 强制收敛，只保留风险等级最高的

Verification 阶段：
  验证 ≤ 20%，结论必须明确：完成 / 未完成 / 部分完成
  禁止在 Verification 里继续扩展实现内容

硬性禁止：
  ✗ 分析过程中发现新问题 → 禁止当场处理，记录到 Additional Findings
  ✗ 执行完成后继续优化 → 满足 Completion Criteria 立即停止，进入 Verification
  ✗ 任意阶段无限展开分析而不推进执行
```
...
**P0 物料完成判定**：
```
AI 已输出该物料的完整内容，且满足以下任一条件：
  · 用户明确确认（如「好的」「没问题」「确认」）
  · 用户在该物料基础上直接推进下一步操作（隐含确认）

⚠️ 以下情况不视为完成：
  · 用户无任何回复（沉默不等于确认）
  · 用户仅继续提问（追问说明物料尚未完整）

判定为完成后 → Memory Diff [静默]：progress.md Stage Deliverables[物料名].状态: □ → ✓
```
...
**触发时机**：progress.md 的 ## Stage Deliverables 中所有 P0 物料状态 = ✓ 时，
输出确认门，等待用户明确确认后才执行 Step 1-4。
